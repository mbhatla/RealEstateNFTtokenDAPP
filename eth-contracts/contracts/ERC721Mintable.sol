...
contract Pausable is Ownable {
    ...
    function changePauseStatus(bool status) public onlyOwner {

        require(status != _paused , "Pause status sent is existing already" );
        _paused=status;
        if (_paused){
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(_tokenOwner[tokenId] != to, "Already the token owner");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == this.getOwner() || isApprovedForAll(ownerOf(tokenId), msg.sender), "Sender is not approved");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(owner != address(0), "Approval does not exist");
        return owner;
    }
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, "Sender is same as approval address");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "Owner or Approved not sending Tx");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "Check on ERC721  failed");
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!(_exists(tokenId)) && to != address(0), " Token exists or the address is invalid");
...
contract ERC721Enumerable is ERC165, ERC721 {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        require(index < balanceOf(owner), "Index is more than balance");
    ...
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "Index is more than total supply");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), "Token id does not exist in set URI");
